import { useState, useEffect } from 'react';
import Sidebar from './components/Sidebar';
import Header from './components/Header';
import StatsSummary from './components/StatsSummary';
import FilterBar from './components/FilterBar';
import MenuList from './components/MenuList';
import MenuForm from './components/MenuForm';
import TabularMenuList from './components/TabularMenuList';
import CategoriesView from './components/CategoriesView';
import ReportsView from './components/ReportsView';
import './App.css';

const initialData = [
  { id: "1", name: "Nasi Goreng Spesial", category: "Makanan", price: 25000, status: "Tersedia", image: "https://images.unsplash.com/photo-1512058564366-18510be2db19?w=500&auto=format&fit=crop&q=60" }, 
  { id: "2", name: "Chicken Steak Panggang", category: "Makanan", price: 35000, status: "Tersedia", image: "https://images.unsplash.com/photo-1598515214211-89d3c73ae83b?w=500&auto=format&fit=crop&q=60" }, 
  { id: "3", name: "Es Teh Manis", category: "Minuman", price: 5000, status: "Tersedia", image: "https://images.unsplash.com/photo-1556679343-c7306c1976bc?w=500&auto=format&fit=crop&q=60" }, 
  { id: "4", name: "Pizza", category: "Makanan", price: 40000, status: "Habis", image: "https://images.unsplash.com/photo-1593560708920-61dd98c46a4e?w=500&auto=format&fit=crop&q=60" },
  { id: "5", name: "Jus Jeruk", category: "Minuman", price: 15000, status: "Habis", image: "https://images.unsplash.com/photo-1621506289937-a8e4df240d0b?w=500&auto=format&fit=crop&q=60" },
  { id: "6", name: "Kopi", category: "Minuman", price: 20000, status: "Tersedia", image: "https://images.unsplash.com/photo-1509042239860-f550ce710b93?w=500&auto=format&fit=crop&q=60" }
];


function App() {
  // Persistensi Data dengan localStorage
  const [menus, setMenus] = useState(() => {
    try {
      const savedMenus = localStorage.getItem('resto_menus');
      return savedMenus ? JSON.parse(savedMenus) : initialData;
    } catch (e) {
      console.error("Gagal memuat menu dari localStorage:", e);
      return initialData;
    }
  });

  // State untuk Tab Aktif di Sidebar
  const [activeTab, setActiveTab] = useState('dashboard');

  // State untuk Filter & Pencarian Real-time (hanya untuk dashboard)
  const [search, setSearch] = useState('');
  const [filterCategory, setFilterCategory] = useState('Semua Kategori');
  const [filterStatus, setFilterStatus] = useState('Semua Status');

  // State untuk Kontrol Form (Tambah / Edit)
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [currentMenu, setCurrentMenu] = useState(null); // Jika berisi object, berarti sedang MODE EDIT

  // State untuk Toast Notification
  const [toast, setToast] = useState({ show: false, message: '', type: 'success' });

  // Sinkronisasi dengan localStorage menggunakan useEffect
  useEffect(() => {
    localStorage.setItem('resto_menus', JSON.stringify(menus));
  }, [menus]);

  // Handler untuk menampilkan toast
  const showToast = (message, type = 'success') => {
    setToast({ show: true, message, type });
  };

  // Tutup toast otomatis setelah 3 detik
  useEffect(() => {
    if (toast.show) {
      const timer = setTimeout(() => {
        setToast({ show: false, message: '', type: 'success' });
      }, 3000);
      return () => clearTimeout(timer);
    }
  }, [toast.show]);

  // Handler CRUD: CREATE & UPDATE
  const handleSaveMenu = (menuData) => {
    // Fallback URL jika kosong
    const defaultImage = menuData.category === 'Makanan' 
      ? "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=500&auto=format&fit=crop&q=60" 
      : "https://images.unsplash.com/photo-1536935338788-846bb9981813?w=500&auto=format&fit=crop&q=60";
    
    const finalImage = menuData.image && menuData.image.trim() !== '' ? menuData.image.trim() : defaultImage;
    const finalMenuData = { ...menuData, image: finalImage };

    if (currentMenu) {
      // Logic UPDATE (Edit)
      setMenus(menus.map(item => item.id === currentMenu.id ? { ...item, ...finalMenuData } : item));
      showToast("Menu berhasil diperbarui!", "success");
    } else {
      // Logic CREATE (Tambah Baru)
      const newMenu = {
        id: String(+new Date()),
        ...finalMenuData
      };
      setMenus([newMenu, ...menus]);
      showToast("Menu baru berhasil ditambahkan!", "success");
    }
    handleCloseForm();
  };

  // Handler CRUD: DELETE dengan window.confirm()
  const handleDeleteMenu = (id) => {
    const confirmDelete = window.confirm("Apakah Anda yakin ingin menghapus menu ini secara permanen?");
    if (confirmDelete) {
      setMenus(menus.filter(item => item.id !== id));
      showToast("Menu berhasil dihapus secara permanen!", "danger");
    }
  };

  // Handler CRUD: Toggle status stok secara cepat dari tabel
  const handleToggleStatus = (id) => {
    setMenus(menus.map(item => {
      if (item.id === id) {
        const nextStatus = item.status === 'Tersedia' ? 'Habis' : 'Tersedia';
        return { ...item, status: nextStatus };
      }
      return item;
    }));
    showToast("Status ketersediaan menu berhasil diubah!", "success");
  };



  // Handler untuk memicu Mode Edit
  const handleEditClick = (menu) => {
    setCurrentMenu(menu);
    setIsFormOpen(true);
  };

  const handleCloseForm = () => {
    setIsFormOpen(false);
    setCurrentMenu(null);
  };

  // Logika Filter & Search Real-time (READ - untuk dashboard grid)
  const filteredMenus = menus.filter(menu => {
    const matchesSearch = menu.name.toLowerCase().includes(search.toLowerCase());
    const matchesCategory = filterCategory === 'Semua Kategori' || menu.category === filterCategory;
    const matchesStatus = filterStatus === 'Semua Status' || menu.status === filterStatus;
    return matchesSearch && matchesCategory && matchesStatus;
  });

  return (
    <div className="dashboard-container">
      <Sidebar activeTab={activeTab} onTabChange={setActiveTab} />
      
      <main className="main-content">
        <Header 
          onAddMenuClick={() => { setCurrentMenu(null); setIsFormOpen(true); }} 
          showAddButton={activeTab === 'dashboard'} 
        />
        
        {activeTab === 'dashboard' && (
          <FilterBar 
            search={search} setSearch={setSearch}
            filterCategory={filterCategory} setFilterCategory={setFilterCategory}
            filterStatus={filterStatus} setFilterStatus={setFilterStatus}
          />
        )}

        <div className="content-layout">
          {/* View 1: Dashboard Grid View */}
          {activeTab === 'dashboard' && (
            <div className="menu-section">
              <MenuList 
                menus={filteredMenus} 
                onEditClick={handleEditClick} 
                onDeleteClick={handleDeleteMenu} 
              />
            </div>
          )}

          {/* View 2: Tabular Menu List View */}
          {activeTab === 'menu-list' && (
            <div className="menu-section">
              <TabularMenuList 
                menus={menus} 
                onEditClick={handleEditClick} 
                onDeleteClick={handleDeleteMenu}
                onToggleStatus={handleToggleStatus}
              />
            </div>
          )}

          {/* View 3: Categories Analytical View */}
          {activeTab === 'categories' && (
            <div className="menu-section">
              <CategoriesView menus={menus} />
            </div>
          )}

          {/* View 4: Reports View */}
          {activeTab === 'reports' && (
            <div className="menu-section">
              <ReportsView menus={menus} />
            </div>
          )}

          {/* Sidebar Floating Form Panel (Hanya aktif untuk Tab Dashboard dan Daftar Menu) */}
          {(activeTab === 'dashboard' || activeTab === 'menu-list') && (
            <div className="side-section">
              {isFormOpen && (
                <MenuForm 
                  key={currentMenu ? currentMenu.id : 'new'}
                  onSave={handleSaveMenu} 
                  onClose={handleCloseForm} 
                  currentMenu={currentMenu} 
                />
              )}
              {activeTab === 'dashboard' && <StatsSummary menus={menus} />}
            </div>
          )}
        </div>
      </main>

      {/* Toast Notification */}
      {toast.show && (
        <div className={`toast-notification ${toast.type}`}>
          <div className="toast-body">
            <span>{toast.type === 'success' ? '✅' : '🗑️'} {toast.message}</span>
            <button className="toast-close-btn" onClick={() => setToast({ ...toast, show: false })}>&times;</button>
          </div>
        </div>
      )}
    </div>
  );
}

export default App;